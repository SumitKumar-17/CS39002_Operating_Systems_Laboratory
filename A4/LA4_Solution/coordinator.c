...
void genblocks(int B[9][9], int bfd[9][2]) {
  int b, i, j;
  char bno[8], bfdin[16], bfdout[16], r1fd[16], r2fd[16], c1fd[16], c2fd[16],
      geometry[64], title[16];
  ...
  for (b = 0; b < 9; ++b) {
    ...
    if (!fork()) {
      execlp("xterm", "xterm", "-T", title, "-fa", "Monospace", "-fs", "15",
             "-geometry", geometry, "-bg", "#331100", "-e", "./block", bno,
             bfdin, bfdout, r1fd, r2fd, c1fd, c2fd, NULL);
    }
  }
}
  ...
  for (i = 0; i < 3; ++i) {
    for (j = 0; j < 3; ++j) {
      ...
      printf("N %d %d %d %d %d %d %d %d %d\n", B[3 * i][3 * j],
             B[3 * i][3 * j + 1], B[3 * i][3 * j + 2], B[3 * i + 1][3 * j],
             B[3 * i + 1][3 * j + 1], B[3 * i + 1][3 * j + 2],
             B[3 * i + 2][3 * j], B[3 * i + 2][3 * j + 1],
             B[3 * i + 2][3 * j + 2]);
      close(1);
      dup(dupout);
    }
