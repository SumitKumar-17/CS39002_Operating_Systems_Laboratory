...
int isempty(readyQ Q) { return (Q.F == (Q.B + 1) % Q.MAXSIZE); }

int isfull(readyQ Q) { return (Q.F == (Q.B + 2) % Q.MAXSIZE); }
...
int eventcmp(job *J, int E1, int E2) {
  if (J[E1].time < J[E2].time)
    return -1;
  if (J[E1].time > J[E2].time)
    return 1;
  if (J[E1].state < J[E2].state)
    return -1;
  if (J[E1].state > J[E2].state)
    return 1;
  if (J[E1].srno < J[E2].srno)
    return -1;
  if (J[E1].srno > J[E2].srno)
    return 1;
  return 0;
    ...
    case TIMEOUT:
      if (J[p].rbt == 0) {
        if (J[p].bno == J[p].nb - 1) {
          ...
          for (i = 0; i < J[p].nb; ++i)
            tbt += J[p].CPUB[i];
          for (i = 0; i < J[p].nb - 1; ++i)
            tbt += J[p].IOB[i];
          wtt = tat - tbt;
          printf("%-10d: Process %6d exits. Turnaround time = %4d (%3.0lf%%), "
                 "Wait time = %d\n",
